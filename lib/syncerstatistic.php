...
class SyncerStatistic implements ISyncer
{
    private $siteId;
...
//            Debug::dump('Site prod');
//            Debug::dump($arProd);
//            Debug::dump('1C prod');
//            Debug::dump($ar1CProds[$arProd['XML_ID']]);

            $arProdFields = [
                'QUANTITY' => floatval($ar1CProds[$arProd['XML_ID']]['Количество']),
...
//            Debug::dump('final prod');
//            Debug::dump($arProdFields);

            $item = $basket->createItem('catalog', $arProd['ID']);
            $item->setFields($arProdFields);

            unset($ar1CProds[$arProd['XML_ID']]);
        }

        //$basket->save();

        // логируем ошибки по ненайденным товарам на сайте
        foreach ($ar1CProds as $ar1CProd) {
            Tables\SyncHistoryTable::add([
                'name' => 'statistic-create-prod',
                'operation' => 'import',
                'result' => 'error',
                'msg' => 'Из заказа ' . $arData['ЗаказКлиента'] . ' на сайте отсутствует товар [' . implode('; ', $ar1CProd) . ']',
            ]);
        }
...
//        Debug::dump('vvvvvvvvvvvvvvvvvvvvvvvvvv');
//        Debug::dump($arCurrentItems);
//        Debug::dump('^^^^^^^^^^^^^^^^^^^^^^^^^^');
        $ar1CProds = $arData['Товары'];
...
//        return true;
        ...
        // получаем существующие на сайте заказы по XML_ID
//        Debug::dump($arData['CODES']);
        $arExistedOrders = [];
        $arOrder = [];
        ...
        while ($arRes = $dbRes->GetNext()) {
            $arExistedOrders[$arRes['XML_ID']] = $arRes;
        }
